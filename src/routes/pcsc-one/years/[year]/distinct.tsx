...
import { fetchYearTracksDistinct } from '~/pcsc/server/years';

export default function PCSCOneYearDistinctDetail() {
  const params = useParams();
  const year = () => parseInt(params.year || '0', 10);

  const [tracks] = createResource(year, async (y) => {
    return await fetchYearTracksDistinct(y);
  });
  ...
  const description = () => {
    ...
    return `${count} track${count !== 1 ? 's' : ''} from this year (one per artist)`;
  };
  ...
  return (
    <>
      ...
      >
        <Show when={tracks()}>
          <PcscTitle headline={`Year ${year()} (distinct)`} description={description()} />
